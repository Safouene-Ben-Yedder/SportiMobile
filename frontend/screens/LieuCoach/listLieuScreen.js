...
export default function ListLieuScreen({ navigation }) {
    ...
    return (
      <HStack flex="1" pl="2">
        <Pressable
          ...
          onPress={() =>
            navigation.navigate("Modifier un lieu", { data: data.item })
          }
          _pressed={{
            opacity: 0.5,
          }}
  ...
  const renderItem = ({ item, index }) => (
    <Box>
      <Pressable
        onPress={() => navigation.navigate("Afficher un lieu", { data: item })}
        _dark={{
          bg: "coolGray.800",
        }}
        _light={{
